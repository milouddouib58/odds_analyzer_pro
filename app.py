...
.rec-strong {
    ...
    box-shadow: 0 0 25px rgba(0,255,136,0.2), inset 0 0 15px rgba(0,255,136,0.03);
    padding: 25px;
    border-radius: 18px;
    margin-top: 12px;
    color: white;
...
.rec-value {
    ...
    box-shadow: 0 0 25px rgba(77,121,255,0.2), inset 0 0 15px rgba(77,121,255,0.03);
    padding: 25px;
    border-radius: 18px;
    margin-top: 12px;
    color: white;
...
# ──────────────────────────────────────────────────────
def remove_margin(odds_list: list) -> list | None:
...
def calculate_stake(true_prob: float, odds: float, bankroll: float, fraction: float) -> float:
    """حساب المبلغ المقترح للرهان مع ضمانات السلامة"""
...
def get_sharp_consensus(bookmakers: list, market_key: str, point: float | None = None) -> dict:
    """
    استخراج الإجماع الحاد من الكتب الذكية (Sharp Books)
    يُعيد: {outcomes: [...], true_probs: [...], vig: float, book_used: str, n_books: int, spread: float}
    """
    # أولاً: ابحث عن Pinnacle (الأكثر موثوقية)
    ...
    # ثانياً: متوسط مرجّح لكل كتب السوق المتاحة
    all_book_probs = {}
    all_book_names = []
    count_books = 0
    for b in bookmakers:
        for m in b.get("markets", []):
            ...
            if probs:
                count_books += 1
                all_book_names.append(b.get("title", ""))
                for i, o in enumerate(outcomes):
                    key = o["name"]
                    all_book_probs[key] = all_book_probs.get(key, []) + [probs[i]]
    if all_book_probs and count_books >= 2:
        names = list(all_book_probs.keys())
        avg_probs = [np.mean(all_book_probs[k]) for k in names]
        total = sum(avg_probs)
        norm_probs = [p / total for p in avg_probs]
        # متوسط الأودز الخام
        raw_odds_avg = []
        for k in names:
            raw_o = []
            for b in bookmakers:
                for m in b.get("markets", []):
                    ...
                    outs = m.get("outcomes", [])
                    if market_key == "totals" and point is not None:
                        outs = [o for o in outs if abs(o.get("point", 0) - point) < 0.01]
                    for o in outs:
                        if o["name"] == k:
                            raw_o.append(o["price"])
            raw_odds_avg.append(np.mean(raw_o) if raw_o else 2.0)
        return {
            "outcomes": names,
            ...
            "n_books": count_books,
            "odds_spread": max(raw_odds_avg) - min(raw_odds_avg) if raw_odds_avg else 0,
        }
    return {}

def get_target_book_odds(bookmakers: list, market_key: str, target_book: str, point: float | None = None) -> dict:
...
def detect_line_value(true_prob: float, book_odds: float, threshold: float = 0.03) -> dict:
    """
    كشف قيمة الخط: هل السوق يخطئ في التسعير؟
    يُعيد: {has_value, ev, edge_pct, confidence}
    """
    ev = calculate_ev(true_prob, book_odds)
    implied_prob = 1.0 / book_odds if book_odds > 1.0 else 1.0
    edge = true_prob - implied_prob  # الميزة الحقيقية
...
def get_available_totals_points(bookmakers: list) -> list:
    """استخراج كل نقاط Over/Under المتاحة في السوق"""
    points = set()
    for b in bookmakers:
        for m in b.get("markets", []):
            if m["key"] == "totals":
...
def generate_ai_narrative(
    ...
    # 1. تحليل الكفاءة السوقية
    if n_books >= 7:
        lines.append(f"📊 **إجماع سوقي قوي:** {n_books} شركة مراهنة تُسعّر هذه المباراة بهامش {vig:.1f}٪ — الأرقام موثوقة جداً.")
    # 2. السيناريو الرئيسي
    if fav and is_one_sided:
        loc = "أمام جمهوره" if fav_home else "خارج أرضه"
        lines.append(
            f"⚡ **سيناريو الهيمنة:** {fav} يدخل هذه المباراة بتفوق لا لبس فيه ({fav_prob*100:.0f}٪ احتمال الفوز وفق السوق الذكي). "
            f"المنافس {underdog} يُعاني من ضعف في مواجهة الفرق القوية {loc}."
        )
        if high_scoring_expected:
            lines.append(f"🔥 **البيانات تتحدث:** احتمال عالٍ لمباراة مثيرة بأهداف كثيرة — السوق يُسعّر فوزاً ساحقاً لـ{fav} مع تضخم هجومي واضح.")
        elif low_scoring_expected:
            lines.append(f"🔒 **الدفاع يحكم:** رغم الهيمنة المتوقعة، السوق يُشير لمباراة تكتيكية بأهداف محدودة. كسب نظيف (1-0 أو 2-0) هو السيناريو الأكثر منطقاً.")
    elif fav and not is_one_sided:
        lines.append(
            f"📌 **أفضلية معتدلة:** {fav} يتقدم بأفضلية ({fav_prob*100:.0f}٪) لكنها ليست حاسمة. "
            f"الشركات الذكية تُعطي {underdog} فرصة حقيقية للمفاجأة ({dog_prob*100:.0f}٪)."
        )
    elif is_balanced:
        lines.append(
            f"⚖️ **مباراة القدر المتساوي:** الأرقام تُشير لتعادل حقيقي في القوى — السوق عاجز عن الترجيح. "
            f"الانتبه من الوقوع في فخ الرهان على النتيجة، والتركيز على الأهداف أذكى."
        )
    # 3. قراءة سوق الأهداف
    if ou_prob_over > 0.60:
        lines.append(f"📈 **ضغط هجومي:** احتمال {ou_prob_over*100:.0f}٪ لتخطي 2.5 أهداف — كلا الفريقين يمتلكان هجوماً فعالاً وسط دفاع هش.")
    elif ou_prob_over < 0.40:
        lines.append(f"📉 **معركة دفاعية:** احتمال {(1-ou_prob_over)*100:.0f}٪ للبقاء تحت 2.5 أهداف — الدفاع سيد الموقف، والمباراة ستُحسم بهدف أو هدفين.")
    # 4. تحليل Value Bets
    golden_recs = [r for r in recs if r.get("tier") == "golden"]
    if golden_recs:
        for gr in golden_recs:
            lines.append(
                f"💎 **كسر التسعير اكتُشف!** {target_book} تُسعّر '{gr['bet']}' بأودز {gr['odds']:.2f} "
                f"بينما الاحتمال الحقيقي يُستحق أودز {1/gr['true_prob']:.2f}. "
                f"الميزة الحسابية: **+{gr['ev']*100:.1f}٪** — هذا ما يبحث عنه المحترفون."
            )
    # 5. توصية ختامية
    if is_balanced and not golden_recs:
        lines.append("🚫 **تحذير المحترف:** المباراة المتكافئة بلا قيمة واضحة = خطر عالٍ. ننصح بالتجاهل أو الرهان الرمزي فقط.")
    elif fav and is_one_sided and golden_recs:
        lines.append(f"🎯 **الخلاصة الذهبية:** تقاطع بين هيمنة {fav} وخطأ تسعير {target_book} — نادر الحدوث، فرصة لا تُعوَّض.")
    elif fav and not golden_recs:
        lines.append(f"💡 **استراتيجية المحترف:** ابحث عن {fav} في الأكومولاتور بدلاً من الرهان المنفرد لتعظيم العائد مع الحفاظ على الأمان.")
    return "\n\n".join(lines) if lines else "🔍 البيانات غير كافية لتوليد قراءة تحليلية موثوقة."

# ──────────────────────────────────────────────────────
# دوال الرسم البياني
# ──────────────────────────────────────────────────────
def render_prob_comparison_chart(home_team: str, away_team: str, h_true: float, a_true: float, d_true: float,
                                 h_implied: float, a_implied: float, target_book: str, index: int):
    ...
    implied_vals = [h_implied * 100,
                    (1 - h_implied - a_implied) * 100 if h_implied + a_implied <= 1 else 10.0,
                    a_implied * 100]
    implied_vals = [max(0, v) for v in implied_vals]
    fig = go.Figure()
    fig.add_trace(go.Bar(
        name="الاحتمال الحقيقي للسوق",
        x=categories,
        y=true_vals,
        marker_color=["rgba(255,215,0,0.8)", "rgba(150,150,150,0.6)", "rgba(77,121,255,0.8)"],
        marker_line_color=["#FFD700", "#aaa", "#4d79ff"],
        marker_line_width=2,
        text=[f"{v:.1f}%" for v in true_vals],
    ...
    fig.add_trace(go.Bar(
        ...
        marker_line_color=["#ff6464", "#888", "#64ff64"],
        marker_line_width=1,
...
def render_ev_gauge(ev: float, confidence: int, index: int, suffix: str = ""):
    ...
    fig = go.Figure(go.Indicator(
        ...
        title={"text": f"القيمة المتوقعة (EV)<br><span style='font-size:12px;color:rgba(255,255,255,0.5)'>Confidence: {confidence}%</span>",
               "font": {"size": 15, "color": "white", "family": "Tajawal"}},
        number={"suffix": "%", "font": {"size": 28, "color": color}, "valueformat": ".1f"},
        gauge={
            "axis": {"range": [-30, 30], "tickwidth": 1, "tickcolor": "rgba(255,255,255,0.3)",
...
def render_vig_radar(bookmakers_data: list, home_team: str, away_team: str, index: int):
    ...
    fig.update_layout(
        title=dict(text="🔬 مقارنة هوامش شركات المراهنة (كلما قلّ كان أفضل للاعب)",
        ...
        xaxis=dict(showgrid=True, gridcolor="rgba(255,255,255,0.05)", ticksuffix="%", range=[0, max(values) * 1.3]),
...
# ──────────────────────────────────────────────────────
with st.sidebar:
    st.markdown('<div class="sidebar-header">🧠 MASTERMIND PRO<br><small style="font-weight:300;opacity:0.7">نظام تحليل الأودز المتقدم</small></div>', unsafe_allow_html=True)
    api_key_input = st.text_input("🔑 Odds API Key", type="password", placeholder="أدخل مفتاح API الخاص بك")
    ...
    user_bankroll = st.number_input(
        ...
        value=1000.0,
        step=100.0,
        help="إجمالي رصيدك المخصص للمراهنة"
    )
    ...
    kelly_map = {"محافظ جداً": 0.10, "محافظ": 0.20, "متوازن": 0.30, "جريء": 0.45, "مغامر": 0.60}
...
with col_info:
    st.info("⚡ يقوم النظام بمقارنة أودز شركتك مع إجماع السوق الحاد (Pinnacle + متوسط السوق) لاكتشاف أخطاء التسعير.")
...
if run_button:
    ...
    with st.spinner("🔄 جارٍ تحليل السوق... جلب البيانات من خوادم Odds API..."):
        matches = fetch_odds(api_key_input, sport_key)

    if not matches:
        st.error("❌ لم يتم جلب أي بيانات. تحقق من API Key والاتصال بالإنترنت.")
        st.stop()

    st.success(f"✅ تم جلب **{len(matches)}** مباراة — جارٍ التحليل العميق...")

    # ملخص السوق العلوي
    total_books_found = set()
    for m in matches:
    ...
    with cols_summary[2]:
        target_icon = "✅" if has_target else "❌"
        st.metric(f"🎯 {target_bookmaker}", f"{target_icon} {'متاحة' if has_target else 'غير متاحة'}")
    with cols_summary[3]:
        sharp_found = any(
    ...
    for index, match in enumerate(matches[:max_matches_to_show * 3]):  # نحلل أكثر لنصفي
        ...
        if h_true == 0 and a_true == 0:
            # حاول بالترتيب الافتراضي
        ...
        if ou_consensus and len(ou_consensus.get("true_probs", [])) >= 2:
            for i, name in enumerate(ou_consensus["outcomes"]):
                if "Over" in name or "over" in name:
                    ou_over_true = ou_consensus["true_probs"][i]
                elif "Under" in name or "under" in name:
                    ou_under_true = ou_consensus["true_probs"][i]

        # ── بناء التوصيات ──
        recs = []
        # تحليل H2H
        h_implied = 1.0 / target_1x2.get(home_team, 100)
        a_implied = 1.0 / target_1x2.get(away_team, 100)
        for team_name, true_prob, book_odds_key in [
            (home_team, h_true, home_team),
            (away_team, a_true, away_team),
        ]:
            if book_odds_key not in target_1x2:
                continue
            book_odds = target_1x2[book_odds_key]
            ...
            stake = calculate_stake(true_prob, book_odds, user_bankroll, active_fraction)
            stake = round(stake)
            rec_base = {
                "bet": f"فوز {team_name}",
                "odds": book_odds,
            ...
            if value_info["has_value"] and value_info["ev"] > min_ev_threshold:
                # قيمة حقيقية ممتازة (خطأ تسعير)
                rec_base["label"] = "💎 خطأ تسعير مكتشف!"
                rec_base["tier"] = "golden"
                rec_base["stake"] = round(min(stake * 1.25, user_bankroll * MAX_STAKE_PCT))
                rec_base["stake"] = max(rec_base["stake"], MIN_STAKE)
        ...
        if ou_consensus and target_ou:
            for ou_name, ou_true_p, ou_label_ar in [
                ("Over", ou_over_true, f"أكثر من {best_point} أهداف"),
                ("Under", ou_under_true, f"أقل من {best_point} أهداف"),
            ]:
                book_ou_odds = target_ou.get(ou_name, 0)
                if not book_ou_odds or not (min_odds_filter <= book_ou_odds <= max_odds_filter):
                    continue
                ou_value = detect_line_value(ou_true_p, book_ou_odds, threshold=min_ev_threshold)
                ou_stake = calculate_stake(ou_true_p, book_ou_odds, user_bankroll, active_fraction)
                ou_stake = max(round(ou_stake), MIN_STAKE)
                ou_rec = {
                    "bet": ou_label_ar,
                    "odds": book_ou_odds,
                    "true_prob": ou_true_p,
                ...
                if ou_value["has_value"] and ou_value["ev"] > min_ev_threshold:
                    ...
                    ou_rec["tier"] = "golden"
                    ou_rec["stake"] = max(round(ou_stake * 1.2), MIN_STAKE)
                    recs.append(ou_rec)
        ...
        recs_sorted = sorted(recs, key=lambda x: (-x["ev"], tier_order.get(x.get("tier", "extra"), 4)))
        final_recs = recs_sorted[:4]  # أفضل 4 توصيات
        ...
        narrative = generate_ai_narrative(
            ...
            h2h_consensus, ou_consensus, target_bookmaker, final_recs
        )
        ...
        if commence_time:
            try:
                ...
                time_str = ""
        book_quality = calculate_market_consensus_strength(h2h_consensus.get("n_books", 0), h2h_consensus.get("vig", 10))

        with st.container():
            st.markdown(f"""
            <div class="match-card">
                <div class="match-title">{home_team} ⚔️ {away_team}</div>
                <div class="match-subtitle">{time_str} &nbsp;|&nbsp; {selected_sport_label} &nbsp;|&nbsp; المرجع: {h2h_consensus.get('book_used','—')} &nbsp;|&nbsp; إجماع السوق: {book_quality}</div>
            </div>
            """, unsafe_allow_html=True)

            # -- الصف الأول: مقارنة الاحتمالات ومقياس EV
            col_chart, col_gauges = st.columns([3, 2])
            with col_chart:
                render_prob_comparison_chart(
                    home_team, away_team, h_true, a_true, d_true,
                    h_implied, a_implied, target_bookmaker, index
                )
            with col_gauges:
                ...
                for i, rec in enumerate(final_recs):
                    with rec_cols[i % n_cols]:
                        ...
                        if tier == "golden":
                            ...
                            css_class = "rec-extra"
                        ev_html = (
                            f'<span class="ev-badge-pos">EV: +{rec["ev"]*100:.1f}%</span>' if rec["ev"] > 0
                            else f'<span class="ev-badge-neg">EV: {rec["ev"]*100:.1f}%</span>'
                        )
                        edge_text = f"ميزة: {rec['edge_pct']:.1f}%" if rec.get("edge_pct") else ""
                        true_odds = f"1/{rec['true_prob']:.2f}" if rec["true_prob"] > 0 else "—"
                        st.markdown(f"""
                        <div class="{css_class}">
                            <div class="rec-label">{rec['label']}</div>
                            <div class="rec-bet-name">{rec['bet']}</div>
                            <div> أودز {target_bookmaker}: <span class="rec-odds-badge">{rec['odds']:.2f}</span> </div>
                            ...
                            <div style="color:rgba(255,255,255,0.3); font-size:11px; margin-top:6px;">
                                Confidence: {rec['confidence']}% | Kelly%: {kelly_criterion(rec['true_prob'], rec['odds'], active_fraction)*100:.1f}%
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
            else:
                st.markdown(
                    '<div class="no-recs">🚫 لا توجد توصيات تتجاوز معايير الجودة لهذه المباراة — الأمان أولاً</div>',
                    unsafe_allow_html=True
                )

            st.markdown("<br><hr style='border-color:rgba(255,215,0,0.1);margin:30px 0;'>", unsafe_allow_html=True)
    ...
    if golden_opportunities:
        ...
        for opp in golden_opportunities:
            ...
            ">
                <div>
                    <span style="color:#FFD700; font-weight:900; font-size:16px;"> {opp.get('home','?')} vs {opp.get('away','?')} </span><br>
                    <span style="color:rgba(255,255,255,0.7); font-size:14px;">{opp['bet']}</span>
                </div>
                <div style="text-align:left;">
                    <span style="color:#FFD700; font-size:20px; font-weight:900;">{opp['odds']:.2f}</span><br>
                    <span style="color:#00ff88; font-size:13px;">EV: +{opp['ev']*100:.1f}%</span>
                </div>
                <div style="text-align:center;">
                    <span style="color:white; font-size:18px; font-weight:700;">{opp['stake']} وحدة</span><br>
                    <span style="color:rgba(255,255,255,0.4); font-size:12px;">حجم الرهان</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
