...
class MatchPredictor:
    ...
    def build_features_from_odds(self, match_data: dict,
        ...
        features['best_home_odds'] = max(home_odds_list) if home_odds_list else 2.0
        features['best_draw_odds'] = max(draw_odds_list) if draw_odds_list else 3.5
        features['best_away_odds'] = max(away_odds_list) if away_odds_list else 2.0

        features['avg_home_odds'] = np.mean(home_odds_list) if home_odds_list else 2.0
        features['avg_draw_odds'] = np.mean(draw_odds_list) if draw_odds_list else 3.5
        features['avg_away_odds'] = np.mean(away_odds_list) if away_odds_list else 2.0

        # 2. الاحتمالات الحقيقية (Shin Method)
        if home_odds_list and draw_odds_list and away_odds_list:
            avg_odds = [
                features['avg_home_odds'],
                features['avg_draw_odds'],
                features['avg_away_odds']
            ]
            ...
            if len(shin_probs) == 3:
                ...
                raw = [1 / o for o in avg_odds]
                s = sum(raw)
                features['shin_home_prob'] = raw[0] / s
                features['shin_draw_prob'] = raw[1] / s if len(raw) > 1 else 0.25
                features['shin_away_prob'] = raw[2] / s if len(raw) > 2 else raw[1] / s
        else:
            features['shin_home_prob'] = 0.4
            features['shin_draw_prob'] = 0.25
            features['shin_away_prob'] = 0.35
        ...
        raw_probs_sum = (
            (1 / features['avg_home_odds'] if features['avg_home_odds'] > 1 else 0) +
            (1 / features['avg_draw_odds'] if features['avg_draw_odds'] > 1 else 0) +
            (1 / features['avg_away_odds'] if features['avg_away_odds'] > 1 else 0)
        )
    ...
    def predict_match(self, home_team: str, away_team: str,
        ...
        draw_value = self.processor.find_value_bets(adjusted_draw, best_draw)
        ...
        draw_kelly = self.processor.kelly_criterion(adjusted_draw, best_draw, bankroll)
        away_kelly = self.processor.kelly_criterion(adjusted_away, best_away, bankroll)

        # أفضل رهان
        value_bets = []
        for outcome, vb, kelly, odds in [
            ('الفوز للمضيف', home_value, home_kelly, best_home),
            ('التعادل', draw_value, draw_kelly, best_draw),
    ...
    if not api_key:
        ...
        if input_mode == "من المباريات المحملة":
            ...
            if st.button("🔍 تحليل المباراة", type="primary"):
                predictor = MatchPredictor()
                result = predictor.predict_match(
                    home_team, away_team, bookie_data
                )
                match = {
                    'home_team': home_team,
                    'away_team': away_team,
